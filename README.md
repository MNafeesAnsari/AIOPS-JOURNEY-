...
# Day 9: Scheduled Automation with Crontab (Cron Jobs)

Today, I mastered task scheduling and background automation in Linux using Crontab.

### Key Learnings:
- **`crontab -e`**: Command used to edit the cron schedule table for the current user.
- **Cron Syntax (`* * * * *`)**: Five-pointer time configuration representing Minute, Hour, Day of Month, Month, and Day of Week.
- **Output Redirection (`>> log 2>&1`)**: Piping background execution logs and errors into a standard log file for analysis. 
